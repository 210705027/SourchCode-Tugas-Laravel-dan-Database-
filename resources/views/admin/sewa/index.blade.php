@extends('layouts.app', ['title' => 'Sewa - Admin'])
    ...
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="flex items-center">
            <button class="text-white focus:outline-none bg-gray-600 px-4 py-2 shadow-sm rounded-md">
                <a href="{{ route('admin.sewa.create') }}">TAMBAH</a>
            </button>
            ...
            <div class="relative mx-4">
                ...
                <form action="{{ route('admin.sewa.index') }}" method="GET">
        ...
        <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
            <div class="inline-block min-w-full shadow-sm rounded-lg overflow-hidden">
                <table class="min-w-full table-auto">
                    <thead class="justify-between">
                        <tr class="bg-gray-600 w-full">
                            <th class="px-16 py-2 text-center">
                                <span class="text-white">ID SEWA</span>
                            </th>
                            <th class="px-16 py-2 text-center">
                                <span class="text-white">Nama</span>
                            </th>
                            <th class="px-16 py-2 text-center">
                                <span class="text-white">JUDUL BUKU</span>
                            </th>
                            <th class="px-16 py-2 text-center">
                                <span class="text-white">TANGGAL PEMINJAMAN</span>
                            </th>
                            <th class="px-16 py-2 text-center">
                                <span class="text-white">TANGGAL PENGEMBALIAN</span>
                            </th>
                            <th class="px-16 py-2 text-center">
                    ...
                    <tbody class="bg-gray-200">
                        ...
                        <tr class="border bg-white">
                            ...
                            <td class="px-16 py-2">
                                {{ $sw->tanggal_peminjaman }}
                            </td>
                            <td class="px-16 py-2">
                                {{ $sw->tanggal_pengembalian }}
                            </td>

                            <td class="px-10 py-2 text-center">
                                <a href="{{ route('admin.sewa.edit', $sw->id) }}"
                                    class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>
                                <a href="{{ route('admin.sewa.show', $sw->id) }}"
                                    class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">SHOW</a>
                                <button onClick="destroy(this.id)" id="{{ $sw->id }}"
                ...
                <div class="bg-white p-3">
                    {{ $sewa->links('vendor.pagination.tailwind') }}
                </div>
                @endif
            </div>
        </div>
